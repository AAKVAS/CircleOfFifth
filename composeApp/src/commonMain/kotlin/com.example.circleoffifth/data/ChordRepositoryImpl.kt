...
class ChordRepositoryImpl(
    ...
    override suspend fun getGameModeByName(modeName: String): Mode {
        return gameDao.getGameModeByName(modeName) ?: run {
            val newMode = Mode(UUID.randomUUID().toString(), modeName)
            gameDao.saveMode(newMode)
            gameDao.getGameModeByName(modeName)!!
        }
    }
